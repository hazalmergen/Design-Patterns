...
public class IteratorPattern {
	...
	public static void main(String[] args) throws Exception {
		...
		if (index >= 0 && index < aggregate.getCount()) {
			AbstractIterator iterator = aggregate.CreateIterator(index);
...
class CollectionIterator implements AbstractIterator {
	...
	public CollectionIterator(Collection collection, int index) {
...
class SecondIterator implements AbstractIterator {
	public void First() {
		_current = index;
	}
	...
	public SecondIterator(Collection collection, int index) {
		_collection = collection;
		this.index = index;
	}
...
interface AbstractAggregate {
	public AbstractIterator CreateIterator(int index) throws Exception;
...
class Collection implements AbstractAggregate {
	...
	public AbstractIterator CreateIterator(int index) throws Exception {
		Scanner reader = new Scanner(System.in);
		System.out.println("Choose your iterator: Press 1 for Regular Iterator, Press 2 for Second Iterator.");
		int n = reader.nextInt();
		if (n == 1)
			return new CollectionIterator(this, index);
		else if (n == 2)
			return new SecondIterator(this, index);
		else
			throw new Exception();
